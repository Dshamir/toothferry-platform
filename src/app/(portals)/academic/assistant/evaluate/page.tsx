...
export default function AssistantEvaluatePage() {
  ...
  function handleSubmitForReview() {
    ...
    evaluateSubmission(selected.id, axes, feedback, false, 'PROF-002', 'Dr. Lasry');
    addNotification({
      type: 'info',
      title: 'Submitted for review',
      message: `Evaluation for ${selected.studentName} sent to Prof. Keren for approval`,
      portal: 'academic',
    });
    setSelected(null);
    ...
    <PageTransition>
      <div className="min-h-screen bg-[var(--color-bg-page)] p-8">
        ...
        <p className="text-[13px] mb-6" style={{ color: 'var(--color-text-secondary)' }}>
          {pendingSubmissions.length} assigned · Evaluations require professor approval
        </p>
            ...
            {selected ? (
              <Card>
                ...
                <div className="flex items-center gap-3">
                  <Button variant="secondary" size="md" onClick={handleSubmitForReview}>
                    Submit for Review
                  </Button>
                  <div
                    className="relative group"
                    title="Requires professor approval to finalize grades"
                  >
                    <Button variant="primary" size="md" className="opacity-50 cursor-not-allowed" disabled>
                      Finalize Grade
                    </Button>
                  </div>
              ...
              <Card>
                <div className="flex flex-col items-center justify-center py-16">
                  ...
                  <div className="text-[12px]" style={{ color: 'var(--color-text-secondary)' }}>
                    Select a submission from the queue to evaluate
                  </div>
                </div>
              </Card>
            )}
